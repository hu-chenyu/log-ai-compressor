...
class TestFilter:
    def test_default_levels_error_fail(self):
        ...
        # 修复缺陷R40：FATAL 级别删除（解析层经 LEVEL_ALIASES 归一
        # ERROR）—— 过滤层回归纯级别集合匹配，无特殊放行分支
        assert not f.match(entry("x", level="FATAL"))
        assert not f.match(entry("x", level="WARN"))
        assert not f.match(entry("x", level="INFO"))

    def test_fatal_normalized_to_error_before_filter(self):
        # 修复缺陷R40：FATAL 经 normalize_level 归一为 ERROR，
        # 由 ERROR 级别准入（原「FATAL 始终放行」特殊分支删除）
        from log_ai_compressor.constants import normalize_level
        assert normalize_level("FATAL") == "ERROR"
        f = EntryFilter(FilterConfig(levels=["ERROR"]))
        assert f.match(entry("x", level=normalize_level("FATAL")))
        # include/exclude 关键字语义照常生效（级别准入后才检查）
        f2 = EntryFilter(FilterConfig(levels=["ERROR"], include=["timeout"]))
        assert not f2.match(entry("out of memory", level="ERROR"))
        f3 = EntryFilter(FilterConfig(levels=["ERROR"], exclude=["boom"]))
        assert not f3.match(entry("boom happened", level="ERROR"))

    def test_default_config_error_fail(self):
        # 修复缺陷R40：默认配置 ERROR+FAIL（FATAL 归一 ERROR）
        assert EntryFilter(FilterConfig()).match(entry("x", level="ERROR"))
        assert EntryFilter(FilterConfig()).match(entry("x", level="FAIL"))
    ...
    def test_config_from_dict_invalid_tolerant(self):
        ...
        # 修复缺陷R40：默认级别 ERROR+FAIL（FATAL 删除归一 ERROR）
